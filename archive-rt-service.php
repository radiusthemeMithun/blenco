...
if( blenco_option('rt_service_style') == 4 ) {
	if ( ( Opt::$layout == 'right-sidebar' || Opt::$layout == 'left-sidebar' ) ) {
		$post_classes = 'col-12';
	} else {
	...
	<div class="container">
		<div class="row">
						...
						<div class="row g-4 item-parent rt-masonry-grid">
							<?php while ( have_posts() ) : the_post(); ?>
								<div class="<?php echo esc_attr( $post_classes );?> item rt-grid-item">
									<?php get_template_part( 'views/content-service', blenco_option( 'rt_service_style' ) ); ?>
								</div>
							<?php endwhile; ?>
						</div>
					<?php else:?>
			...
			if ( is_active_sidebar( Fns::default_sidebar('service') ) ) {
				blenco_sidebar( Fns::default_sidebar('service')  );
			} else {
