...
use RT\Blenco\Modules\Pagination;

class BlencoWcFunctions {
	...
	public function __construct() {
		...
		add_action('woocommerce_share',[__CLASS__,'blenco_product_share']);
	...
	public function loop_shop_per_page(){
		$shop_posts_per_page = blenco_option('products_per_page');
	...
	public function loop_shop_columns(  ) {
		if( isset($_GET["displayview"]) && $_GET["displayview"] == 'list' ) {
			...
			$cols = blenco_option('products_cols_width');
		}
		return $cols;
	}
	//Shop category
	...
	public function sale_flash( $args, $post, $product  ) {
		if ( 'percentage' == blenco_option('wc_sale_label')  ) {
			...
			return sprintf('<span class="text onsale rt-prodcut-badge-1">' . esc_html__( 'Sale!', 'blenco' ) . '</span>', $discount );
		}
	}
	...
	public function flash_sale_countdown_positions( $positions ) {
		...
		$positions['before_add_to_cart']['hooks']= [
			'blenco_shop_layout_before_cart_button'
		];
		$positions['after_add_to_cart']['hooks']= [
			'blenco_shop_layout_after_cart_button'
		];

		return $positions;
	}

	public static function blenco_product_share(  ) {
		echo '<div class="product-single-social-shares-btns">';
		self::blenco_product_social_share();
		echo '</div>';
	}

	public static function blenco_product_social_share(){
		...
		<div class="post-share-btn">
			<ul class="item-social">
				<li class="item-label"><?php esc_html_e( 'Share:', 'blenco' );?></li>
