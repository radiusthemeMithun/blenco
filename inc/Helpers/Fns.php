...
class Fns {
	...
	public static function can_show_post_thumbnail() {
		return apply_filters(
			'blenco_can_show_post_thumbnail',
	...
	public static function get_socials() {
		return apply_filters( 'blenco_socials_icon', [
			'facebook'  => [
				'title' => __( 'Facebook', 'blenco' ),
				'url'   => blenco_option( 'facebook' ),
			],
			'twitter'   => [
				'title' => __( 'Twitter', 'blenco' ),
				'url'   => blenco_option( 'twitter' ),
			],
			'linkedin'  => [
				'title' => __( 'Linkedin', 'blenco' ),
				'url'   => blenco_option( 'linkedin' ),
			],
			'youtube'   => [
				'title' => __( 'Youtube', 'blenco' ),
				'url'   => blenco_option( 'youtube' ),
			],
			'pinterest' => [
				'title' => __( 'Pinterest', 'blenco' ),
				'url'   => blenco_option( 'pinterest' ),
			],
			'instagram' => [
				'title' => __( 'Instagram', 'blenco' ),
				'url'   => blenco_option( 'instagram' ),
			],
			'skype'     => [
				'title' => __( 'Skype', 'blenco' ),
				'url'   => blenco_option( 'skype' ),
			],
			'tiktok'    => [
				'title' => __( 'TikTok', 'blenco' ),
				'url'   => blenco_option( 'tiktok' ),
			],
		] );
	...
	public static function get_team_socials() {
		return apply_filters( 'blenco_team_socials', array(
			'facebook'      => array(
				'label' => esc_html__( 'Facebook', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-facebook',
			),
			'twitter'       => array(
				'label' => esc_html__( 'Twitter', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-x-twitter',
			),
			'linkedin'      => array(
				'label' => esc_html__( 'Linkedin', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-linkedin',
			),
			'skype'         => array(
				'label' => esc_html__( 'Skype', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-skype',
			),
			'youtube'       => array(
				'label' => esc_html__( 'Youtube', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-youtube',
			),
			'pinterest'     => array(
				'label' => esc_html__( 'Pinterest', 'blenco' ),
				'type'  => 'text',
				'icon'  => 'icon-rt-pinterest',
			),
			'instagram'     => array(
				'label' => esc_html__( 'Instagram', 'blenco' ),
				'type'  => 'text',
	...
	public static function sidebar_lists( $default_title = null ) {
		$sidebar_fields            = [];
		$sidebar_fields['default'] = $default_title ?? esc_html__( 'Choose Sidebar', 'blenco' );
	...
	public static function image_placeholder( $name, $total = 1, $type = 'png' ) {
		...
		for ( $i = 1; $i <= $total; $i ++ ) {
			...
			$presets[ $i ] = [
				'image' => blenco_get_img( $image_name ),
				'name'  => __( 'Style', 'blenco' ) . ' ' . $i,
			];
		}

		return apply_filters( 'blenco_image_placeholder', $presets );
	}
	...
	public static function blog_column() {
		...
		} elseif ( blenco_option( 'rt_blog_style' ) === 'list' ) {
			$output = 'col-lg-12';
	...
	public static function get_post_types() {
		...
		$exclude = apply_filters( 'blenco_exclude_post_type', [
			'attachment',
			'revision',
			'nav_menu_item',
			'elementor_library',
	...
	public static function meta_style( $exclude = [] ) {
		$meta_style = [
			'meta-style-default' => __( 'Default From Theme', 'blenco' ),
			'meta-style-border'  => __( 'Border Style', 'blenco' ),
			'meta-style-dash'    => __( 'Before Dash ( — )', 'blenco' ),
			'meta-style-dash-bg' => __( 'Before Dash with BG ( — )', 'blenco' ),
			'meta-style-pipe'    => __( 'After Pipe ( | )', 'blenco' ),
		];
	...
	public static function single_post_style( $exclude = [] ) {
		$meta_style = [
			'1' => __( 'Style 1 (Default From Theme)', 'blenco' ),
			'2' => __( 'Style 2 (Container Width Thumb)', 'blenco' ),
			'3' => __( 'Style 3 (Transparent Thumb)', 'blenco' ),
			'4' => __( 'Style 4 (Content Over on Thumb)', 'blenco' ),
			'5' => __( 'Style 5 (Meta Under Thumb)', 'blenco' ),
		];
	...
	public static function blog_meta_list() {
		return [
			'author'   => __( 'Author', 'blenco' ),
			'date'     => __( 'Date', 'blenco' ),
			'category' => __( 'Category', 'blenco' ),
			'tag'      => __( 'Tag', 'blenco' ),
			'comment'  => __( 'Comment', 'blenco' ),
			'reading'  => __( 'Reading', 'blenco' ),
			'views'  => __( 'Views', 'blenco' ),
		];
	}
	...
	public static function post_share_list() {
		return [
			'facebook'  => __( 'Facebook', 'blenco' ),
			'twitter'   => __( 'Twitter X', 'blenco' ),
			'linkedin'  => __( 'Linkedin', 'blenco' ),
			'pinterest' => __( 'Pinterest', 'blenco' ),
			'whatsapp'  => __( 'Whatsapp', 'blenco' ),
			'youtube'   => __( 'Youtube', 'blenco' ),
		];
	}
	...
	public static function default_sidebar( $id = '' ) {
		$sidebar_lists = [
			'main'   => [
				'id'    => 'rt-sidebar',
				'name'  => __( 'Main Sidebar', 'blenco' ),
				'class' => 'rt-sidebar',
			],
			'single' => [
				'id'    => 'rt-single-sidebar',
				'name'  => __( 'Single Sidebar', 'blenco' ),
				'class' => 'rt-single-sidebar',
			],
			'service' => [
				'id'    => 'rt-service-sidebar',
				'name'  => __( 'Service Sidebar', 'blenco' ),
		...
		if ( class_exists( 'WooCommerce' ) ) {
			$sidebar_lists['woo-archive'] = [
				...
				'name'  => __( 'WooCommerce Archive Sidebar', 'blenco' ),
				'class' => 'woo-archive-sidebar',
			];
			$sidebar_lists['woo-single']  = [
				'id'    => 'rt-woo-single-sidebar',
				'name'  => __( 'WooCommerce Single Sidebar', 'blenco' ),
				'class' => 'woo-single-sidebar',
			];
		}
		$sidebar_lists = apply_filters( 'blenco_sidebar_lists', $sidebar_lists );
		if ( ! $id ) {
	...
	public static function blenco_shop_icons_enable(  ) {
		return blenco_option( 'rt_header_compare' ) || blenco_option( 'rt_header_wishlist' ) || blenco_option( 'rt_header_add_to_cart' ) ? true:false;
	}
}
