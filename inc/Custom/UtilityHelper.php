...
class UtilityHelper {
	...
	public function theme_menu() {
		add_theme_page( esc_html__( 'Theme License', 'blenco' ), esc_html__( 'Theme License', 'blenco' ), 'manage_options', 'rtlc', [ $this, 'create_admin_page' ], null, 99 );
	}
	...
	public function create_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Theme License', 'blenco' ); ?></h1>
			<form method="post" action="options.php">
				<?php
				// This prints out all hidden setting fields.
				settings_fields( 'rt_option_group' );
				do_settings_sections( 'blenco-setting' );
				?>
			</form>
		</div>
		<?php
	...
	public function theme_option() {
		...
		add_settings_field(
			'rt_purchase_code',
			esc_html__( 'Purchase Code', 'blenco' ),
		...
		add_settings_field(
			'rt_license_status',
			esc_html__( 'License Status', 'blenco' ),
	...
	public function license_status_callback() {
		...
		$status_text = esc_html__( 'Not Activated', 'blenco' );

		if ( rtlc_is_valid()['success'] ) {
			$verify      = true;
			$status_text = __( 'Activated', 'blenco' );
		} elseif ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$status_text = __( 'Domain Mismatch', 'blenco' );
		}
	...
	public function license_note_callback() {
		...
		/* translators: Support Center */
			__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our <a href="%s" target="_blank">Support Center</a>', 'blenco' ),
			esc_url( $support )
		);
	...
	public function license_check_callback() {
		printf(
			...
			esc_html__( 'Activate License', 'blenco' )
		);
	}
	...
	public function rtlc_verification() {
		...
		if ( $purchase_code ) {
			...
			$api_url     = "{$rt_license_server}/?theme_name={$blenco}&purchase_code=" . $purchase_code . '&domain_name=' . $domain_name;
