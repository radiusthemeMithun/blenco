...
class DynamicStyles {
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'blenco-dynamic', false, 'blenco-main' );
		wp_enqueue_style( 'blenco-dynamic' );
		wp_add_inline_style( 'blenco-dynamic', $this->minify_css( $dynamic_css ) );
	}
	...
	private function inline_style() {
		...
		$primary_color   	= blenco_option( 'rt_primary_color', '#B8E900' );
		$secondary_color 	= blenco_option( 'rt_secondary_color', '#cef837ff' );
		$tertiary_color    	= blenco_option( 'rt_tertiary_color', '#7c9e03ff' );
		$body_bg_color   	= blenco_option( 'rt_body_bg_color', '#111111' );
		$body_color      	= blenco_option( 'rt_body_color', '#d9d9d9' );
		$border_color      	= blenco_option( 'rt_border_color', '#e7e7e7' );
		$title_color     	= blenco_option( 'rt_title_color', '#111111' );
		$button_color    	= blenco_option( 'rt_button_color', '#ffffff' );
		$button_text_color 	= blenco_option( 'rt_button_text_color', '#000' );
		$meta_color      	= blenco_option( 'rt_meta_color', '#B8E900' );
		$meta_light      	= blenco_option( 'rt_meta_light', '#575757' );
		$gray10          	= blenco_option( 'rt_gray10_color', '#f1f1f1' );
		$gray20          	= blenco_option( 'rt_gray20_color', '#edf5f4' );
	...
	protected function topbar_css() {
		...
		if ( ! empty( $_topbar_active_color ) ) : ?>
			body .site-header .blenco-topbar .topbar-container a:hover:not(.dropdown-menu a:hover),
			body .blenco-topbar #topbar-menu ul ul li.current_page_item > a,
			body .blenco-topbar #topbar-menu ul ul li.current-menu-ancestor > a,
			body .blenco-topbar #topbar-menu ul.blenco-topbar-menu > li.current-menu-item > a,
			body .blenco-topbar #topbar-menu ul.blenco-topbar-menu > li.current-menu-ancestor > a {
			color: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}

			body .site-header .blenco-topbar .topbar-container a:hover:not(.dropdown-menu a:hover svg) svg,
			body .blenco-topbar #topbar-menu ul ul li.current-menu-ancestor > a svg,
			body .blenco-topbar #topbar-menu ul.blenco-topbar-menu > li.current-menu-item > a svg,
			body .blenco-topbar #topbar-menu ul.blenco-topbar-menu > li.current-menu-ancestor > a svg {
	...
	protected function header_css() {
		...
		$logo_dimension     = blenco_option( 'rt_logo_width_height' );
		$logo_mobile_dimension     = blenco_option( 'rt_mobile_logo_width_height' );
		$menu_border_bottom = blenco_option( 'rt_menu_border_color' );
		...
		//Default Menu
		$_menu_color        = blenco_option( 'rt_menu_color' );
		$_menu_active_color = blenco_option( 'rt_menu_active_color' );
		$_menu_bg_color     = blenco_option( 'rt_menu_bg_color' );
		$_sub_menu_bg_color     = blenco_option( 'rt_sub_menu_bg_color' );

		//Transparent Menu
		$_tr_menu_color        = blenco_option( 'rt_tr_menu_color' );
		$_tr_menu_active_color = blenco_option( 'rt_tr_menu_active_color' );

		$_header_border     = blenco_option( 'rt_header_border' );
		$_breadcrumb_border = blenco_option( 'rt_breadcrumb_border' );
		$_preloader_bg_color = blenco_option( 'preloader_bg_color' );
		?>

		<?php //Header Logo CSS ?>
		<?php if ( Opt::$header_width == 'full' ) :
			$h_width = '100%';
			if ( ( $header_width = blenco_option( 'rt_header_max_width' ) ) > 768 ) {
		...
		<?php if ( ! empty( $_menu_color ) ) : ?>
			body .blenco-navigation ul li a,
			body .blenco-offcanvas-drawer ul.menu li a,
			body .blenco-navigation ul li ul li a,
			body .menu-icon-wrapper .menu-search-bar {
				color: <?php echo esc_attr( $_menu_color ) ?>;
			}
			body .main-header-section svg,
			body .blenco-navigation .caret svg {
		...
		<?php if ( ! empty( $_menu_active_color ) ) : ?>
			body .blenco-navigation ul li a:hover,
			body .blenco-navigation ul li.current-menu-item > a,
			body .blenco-navigation ul li.current-menu-ancestor > a,
			body .blenco-offcanvas-drawer ul li.current-menu-ancestor > a,
			body .blenco-offcanvas-drawer ul.menu li a:hover,
			body .blenco-navigation ul li ul li a:hover {
				color: <?php echo esc_attr( $_menu_active_color ) ?>;
			}
			body .blenco-navigation ul li a:hover svg,
			body .blenco-navigation ul li.current-menu-item > a svg,
			body .blenco-navigation ul li.current-menu-ancestor > a svg {
		...
		<?php if ( ! empty( $_sub_menu_bg_color ) ) : ?>
			body .blenco-navigation ul > li > ul,
			body .blenco-navigation ul li.mega-menu > ul.dropdown-menu{
		...
		<?php if ( ! empty( $_tr_menu_color ) ) : ?>
			...
			body.has-trheader .site-header .blenco-navigation *,
			body.has-trheader .site-header .blenco-navigation ul li a {
			...
			body.has-trheader .site-header .blenco-topbar .caret svg,
		...
		<?php if ( ! empty( $_tr_menu_active_color ) ) : ?>
			body.has-trheader .site-header .blenco-navigation ul li a:hover,
			body.has-trheader .site-header .blenco-navigation ul li.current-menu-item > a,
			body.has-trheader .site-header .blenco-navigation ul li.current-menu-ancestor > a {
			...
			body.has-trheader .site-header .blenco-navigation ul li.current-menu-ancestor > a svg,
			body.has-trheader .site-header .blenco-navigation ul li.current-menu-item > a svg {
		...
		<?php if ( ! empty( $menu_border_bottom ) ) : ?>
			body .blenco-topbar,
			body .main-header-section,
			body .blenco-breadcrumb-wrapper {
		...
		<?php if ( ! $_breadcrumb_border ) : ?>
			body .blenco-breadcrumb-wrapper {border-bottom: none;}
		<?php endif; ?>
	...
	protected function breadcrumb_css() {
		$breadcrumb_color          = blenco_option( 'rt_breadcrumb_color' );
		$rt_breadcrumb_hover       = blenco_option( 'rt_breadcrumb_hover' );
		$breadcrumb_active         = blenco_option( 'rt_breadcrumb_active' );
		$rt_breadcrumb_title_color = blenco_option( 'rt_breadcrumb_title_color' );
		$rt_banner_color_opacity = blenco_option( 'rt_banner_color_opacity' );

		$rt_banner_padding_top = blenco_option( 'rt_banner_padding_top' );
		$rt_banner_padding_bottom = blenco_option( 'rt_banner_padding_bottom' );

		$rt_banner_overlay1_color = blenco_option( 'rt_banner_overlay1_color' );
		$rt_banner_overlay2_color = blenco_option( 'rt_banner_overlay2_color' );

		if ( ! empty( $rt_breadcrumb_title_color ) ) { ?>
			.blenco-breadcrumb-wrapper .entry-title {
				color: <?php echo esc_attr( $rt_breadcrumb_title_color ) ?> !important;
			}
		<?php }

		if ( ! empty( $breadcrumb_color ) ) { ?>
			.blenco-breadcrumb-wrapper .breadcrumb a,
			.blenco-breadcrumb-wrapper .entry-breadcrumb span a,
			.has-trheader .blenco-breadcrumb-wrapper .breadcrumb a,
			.blenco-breadcrumb-wrapper .entry-breadcrumb .dvdr {
				color: <?php echo esc_attr( $breadcrumb_color ) ?>;
			}
		<?php }

		if ( ! empty( $rt_breadcrumb_hover ) ) { ?>
			.blenco-breadcrumb-wrapper .breadcrumb a:hover,
			.blenco-breadcrumb-wrapper .entry-breadcrumb span a:hover {
			color: <?php echo esc_attr( $rt_breadcrumb_hover ) ?>;
			}
		<?php }

		if ( ! empty( $breadcrumb_active ) ) { ?>
			.blenco-breadcrumb-wrapper .breadcrumb li.active .title,
			.blenco-breadcrumb-wrapper .breadcrumb a:hover,
			.blenco-breadcrumb-wrapper .entry-breadcrumb span a:hover,
			.blenco-breadcrumb-wrapper .entry-breadcrumb .current-item,
			.has-trheader .blenco-breadcrumb-wrapper .breadcrumb li.active .title,
			.has-trheader .blenco-breadcrumb-wrapper .breadcrumb a:hover {
		...
		if ( ! empty( $rt_banner_overlay1_color && $rt_banner_overlay2_color ) ) { ?>
			.blenco-breadcrumb-wrapper.has-bg::before {
	...
	protected function content_padding_css() {
		...
		protected function footer_css() {
			...
			echo self::css( 'body .site-footer .widget-title, .blenco-footer-2 .site-footer .widget-title', 'color', 'rt_footer_widget_title_color' );
		...
		function site_fonts() {
			...
			$typo_body           = json_decode( blenco_option( 'rt_body_typo' ), true );
			$typo_menu           = json_decode( blenco_option( 'rt_menu_typo' ), true );
			$typo_heading        = json_decode( blenco_option( 'rt_all_heading_typo' ), true );
			$body_font_family    = $typo_body['font'] ?? 'Plus Jakarta Sans';
			...
			foreach ( $heading_fonts as $heading ) {
				$font = json_decode( blenco_option( "rt_heading_{$heading}_typo" ), true );
				if ( ! empty( $font['font'] ) ) {
