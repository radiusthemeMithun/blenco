...
class Extras {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'blenco_custom_pagesize' ],1 );
	...
	public function body_class( $classes ) {
		...
		$classes[] = 'blenco-header-' . Opt::$header_style;
		$classes[] = 'blenco-footer-' . Opt::$footer_style;
		...
		if ( blenco_option( 'rt_tr_header_shadow' ) ) {
		...
		if ( blenco_option( 'rt_sticy_header' ) ) {
			$classes[] = 'has-sticky-header';
		}

		if ( blenco_option( 'rt_sticky_footer' ) ) {
			$classes[] = 'has-sticky-footer';
		}

		if ( blenco_option( 'rt_blend' ) ) {
			$classes[] = 'rt-blend';
		}

		if ( blenco_option( 'rt_remove_admin_bar' ) ) {
			$classes[] = 'remove-admin-bar';
		}

		if ( is_single() && Opt::$single_style ) {
			$classes[] = 'blenco-single-' . Opt::$single_style;
		}
	...
	/*custom team archive */
	function blenco_custom_pagesize( $query ) {
	...
	function menu_customize( $item_id, $item ) {
		// Mega menu
		$_mega_menu = get_post_meta( $item_id, 'blenco_mega_menu', true );
		// Query string
		$menu_query_string = get_post_meta( $item_id, 'blenco_menu_qs', true );
		?>
		...
		<p class="description widefat">
			<label class="widefat" for="blenco-menu-qs-<?php echo esc_attr($item_id); ?>">
				<?php echo esc_html__( 'Query String', 'blenco' ); ?><br>
				<input type="text"
					   class="widefat"
					   id="blenco-menu-qs-<?php echo esc_attr($item_id); ?>"
					   name="blenco-menu-qs[<?php echo esc_attr($item_id); ?>]"
	...
	function menu_modify( $items, $menu, $args ) {
		foreach ( $items as $item ) {
			$menu_query_string = get_post_meta( $item->ID, 'blenco_menu_qs', true );
	...
	public function search_form() {
		...
		<form method="get" class="blenco-search-form" action="' . esc_url( home_url( '/' ) ) . '">
            <div class="search-box">
				<input type="text" class="form-control" placeholder="' . esc_attr__( 'Search here...', 'blenco' ) . '" value="' . get_search_query() . '" name="s" />
				<button class="item-btn" type="submit">
					' . blenco_get_svg( 'search', false ) . '
					<span class="btn-label">' . esc_html__( 'Search', 'blenco' ) . '</span>
				</button>
            </div>
		</form>
		';
