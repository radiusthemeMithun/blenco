	...
	public function register_notice() {
		...
		if ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$message = sprintf(
				...
				__( 'Your license key doesn\'t match your current domain. Please activate the license again for your current domain.', 'blenco' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'blenco' ),
			);
		} else {
			$message = sprintf(
				'<b>%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s <a href="%5$s">%6$s</a></b>',
				__( 'Please activate your theme using Envato', 'blenco' ),
				esc_url( 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ),
				__( 'purchase code', 'blenco' ),
				__( 'to get full theme functionality and customer support.', 'blenco' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'blenco' ),
			);
		}
					...
					if (purchase_code) {
						jQuery.ajax({
							...
							success: function (resp) {
								if (resp === 555) {
									alert('<?php esc_html_e( 'Purchase code already activated for one domain!!!', 'blenco' ); ?>');
								} else {
									if (resp) {
										jQuery('.rtlc-status-btn').html('<?php esc_html_e( 'Activated', 'blenco' ); ?>');
										jQuery('.rtlc-status-btn').removeClass('rtlc-unverified').addClass('rtlc-verified');
									} else {
										alert('<?php esc_html_e( 'Sorry!!! Purchase code does not match', 'blenco' ); ?>');
									}
								}
						...
						alert('<?php esc_html_e( 'Purchase code is required!!!', 'blenco' ); ?>');
					}
				});
			</script>
			<?php
			...
			if ( isset( $_GET['page'] ) && 'fw-backups-demo-content' === $_GET['page'] ) {
				...
				<script type="text/javascript">
					jQuery("#fw-ext-backups-demo-list .theme-actions a").on("click", function (e) {
						e.preventDefault();
						alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to install demo data.', 'blenco' ); ?>');
						return false;
					});
				</script>
				<?php
				...
				<script type="text/javascript">
					jQuery(".row-actions a").on("click", function (e) {
						if (jQuery(this).closest('td').next().has('span').length > 0) { //find pre packaged
							e.preventDefault();
							alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to use this plugin.', 'blenco' ); ?>');
							return false;
						}
					});
...
if ( ! function_exists( 'rtlc_is_valid' ) ) {
	...
	function rtlc_is_valid() {
		...
		$blenco      = $theme_info->get( 'Name' );

		$blenco      = strtolower( trim( preg_replace( '/[^A-Za-z0-9-]+/', '-', $blenco ) ) );
		$get_option = get_option( 'rt_licenses' );

		if ( isset( $get_option[ $blenco . '_license_key' ] ) ) {
			return [
				'success'      => true,
				'domain_match' => true,
			];
		} elseif ( isset( $get_option[ $blenco . '_license' ] ) && isset( $get_option[ $blenco . '_license' ]['key'] ) ) {
			$domain_name = rtlc_get_domain_name();
			$domain      = $get_option[ $blenco . '_license' ]['domain'];
