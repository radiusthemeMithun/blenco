...
use RT\Blenco\Modules\PostShare;

/*Allow HTML for the kses post*/
function blenco_html( $html, $context = '' ) {
...
if ( ! function_exists( 'blenco_custom_menu_cb' ) ) {
	/**
...
if ( ! function_exists( 'blenco_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function blenco_menu_icons_group( $args = [] ) {
		$default_args = [
			'hamburg'       => blenco_option( 'rt_header_bar' ),
			'search'        => blenco_option( 'rt_header_search' ),
			'login'         => blenco_option( 'rt_header_login_link' ),
			'button'        => blenco_option( 'rt_get_started_button' ),
			'button_label'  => blenco_option( 'rt_get_started_label' ),
			'button_link'   => blenco_option( 'rt_get_started_button_url' ),
			'has_separator' => blenco_option( 'rt_header_separator' ),
			'compare'    	=> blenco_option( 'rt_header_compare' ),
			'wishlist'    	=> blenco_option( 'rt_header_wishlist' ),
			'add_to_cart' 	=> blenco_option( 'rt_header_add_to_cart' )
		];
		...
		<div class="menu-icon-wrapper ml-auto">
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				...
				<?php endif; ?>
				<?php if ( $args['hamburg'] ) : ?>
					<?php blenco_hanburger( 'desktop-hamburg' ); ?>
				<?php endif; ?>
				<?php blenco_hanburger( 'mobile-hamburg' ); ?>
			</ul>
		</div>
		<?php
	}
...
if ( ! function_exists( 'blenco_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function blenco_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'blenco_readmore_text' ) ) {
	/**
	 * Creates continue reading text.
	 *
	 * @return string
	 */
	function blenco_readmore_text() {

		if ( empty( blenco_option( 'rt_blog_read_more' ) ) ) {
			return;
		}
		return sprintf(
			'%s %s',
			esc_html( blenco_option( 'rt_blog_read_more' ) ),
			the_title( '<span class="screen-reader-text">', '</span>', false )
		);
	}
}

if ( ! function_exists( 'blenco_list_item_separator' ) ) {
	/**
	 ...
	 * @since 6.0.0
	 */
	function blenco_list_item_separator() {
...
if ( ! function_exists( 'blenco_posted_in' ) ) {
	/**
	 * Prints HTML with category list information about theme categories.
	 * @return string
	 */
	function blenco_posted_in( $type = 'category' ) {
		$categories_list = get_the_category_list( blenco_list_item_separator() );
		if ( 'tag' === $type ) {
			$categories_list = get_the_tag_list( '', blenco_list_item_separator() );
		}
		if ( $categories_list ) {
			return sprintf(
...
if ( ! function_exists( 'blenco_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 * @return string
	 */
	function blenco_posted_on() {
		$time_string = sprintf(
...
if ( ! function_exists( 'blenco_posted_by' ) ) {
	/**
...
if ( ! function_exists( 'blenco_get_svg' ) ) {
	/**
	 * Get svg icon
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function blenco_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'blenco_svg_icon_list', [
...
if ( ! function_exists( 'blenco_get_file' ) ) {
	/**
	 * Get File Path
	 *
	 * @param $path
	 ...
	 * @return string
	 */
	function blenco_get_file( $path, $return_path = false ): string {
...
if ( ! function_exists( 'blenco_get_img' ) ) {
	/**
	 * Get Image Path
	 *
	 * @param $filename
	 ...
	 * @return string|void
	 */
	function blenco_get_img( $filename, $echo = false, $image_meta = '' ) {
		$path      = '/assets/images/' . $filename;
		$image_url = blenco_get_file( $path );
...
if ( ! function_exists( 'blenco_get_css' ) ) {
	/**
	 * Get CSS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function blenco_get_css( $filename, $check_rtl = false ) {
		$min    = WP_DEBUG ? '.css' : '.css';
		$is_rtl = $check_rtl && is_rtl() ? 'css-rtl' : 'css';
		$path   = "/assets/$is_rtl/" . $filename . $min;

		return blenco_get_file( $path );
	}
}

if ( ! function_exists( 'blenco_get_js' ) ) {
	/**
	 * Get JS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function blenco_get_js( $filename ) {
...
if ( ! function_exists( 'blenco_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function blenco_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'blenco_get_social_html' ) ) {
	/**
	 * Get Social markup
	 *
	 * @param $color
	...
	function blenco_get_social_html( $color = '' ) {
		ob_start();
...
if ( ! function_exists( 'blenco_site_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function blenco_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = blenco_option( 'rt_logo' );
		$logo_light      = blenco_option( 'rt_logo_light' );
		$logo_mobile     = blenco_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'blenco_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function blenco_footer_logo() {
		$main_logo  = blenco_option( 'rt_logo' );
		$logo_light = blenco_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'blenco_scroll_top' ) ) {
	/**
	 * Back-to-top button
	 * @return void
	 */
	function blenco_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( blenco_option( 'rt_back_to_top' ) ) {
			?>
...
if ( ! function_exists( 'blenco_meta_icons' ) ) {
	/**
	 * Get meta icons
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function blenco_meta_icons( $name ) {
		if ( ! $name ) {
			return;
		}
		// $icon_list = [
...
if ( ! function_exists( 'blenco_reading_time' ) ) {
	/**
	 * Post reading time
	 */
...
if( !function_exists( 'rt_post_views' )){
	...
	function rt_post_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$blenco_view_html = esc_html__( 'View' , 'blenco' );
		} else {
			$blenco_view_html = esc_html__( 'Views' , 'blenco' );
		}

		$blenco_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $blenco_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $blenco_views_html.'</span> ';
	}
}

if ( ! function_exists( 'blenco_post_meta' ) ) {
	/**
	 * Get post meta
	 *
	 * @return string
	 */
	function blenco_post_meta( $args ) {
		$default_args = [
			'with_list'     => true,
			'with_icon'     => true,
			'include'       => [],
			'class'         => '',
			'author_prefix' => __( 'By', 'blenco' )
		];
		...
		$_meta_data['author']   = blenco_posted_by( $args['author_prefix'] );
		$_meta_data['date']     = blenco_posted_on();
		$_meta_data['category'] = blenco_posted_in();
		$_meta_data['tag']      = blenco_posted_in( 'tag' );
		$_meta_data['comment']  = esc_html( $comments_text );
		$_meta_data['reading']  = blenco_reading_time();
		...
		foreach ( $meta_list as $key ) {
			...
			$output .= $args['with_icon'] ? blenco_meta_icons( $key ) : null;
			$output .= $meta;
...
if ( ! function_exists( 'blenco_post_thumbnail' ) ) {
	/**
	 * Displays post thumbnail.
	 * @return void
	 */
	function blenco_post_thumbnail( $size = 'full', $thumb_date = false ) {
...
if ( ! function_exists( 'blenco_post_single_thumbnail' ) ) {
	/**
	 * Display post details thumbnail
	 * @return void
	 */
	function blenco_post_single_thumbnail( $size = 'full' ) {
		if ( ! Fns::can_show_post_thumbnail() ) {
			return;
		}
		?>
		...
		<?php } else { ?>
			...
			<?php if ( wp_get_attachment_caption( get_post_thumbnail_id() ) && blenco_option( 'rt_single_caption_visibility' ) == 1 ) : ?>
				<figcaption class="wp-caption-text">
					<span><?php blenco_html( wp_get_attachment_caption( get_post_thumbnail_id() ) ); ?></span>
				</figcaption>
			<?php endif; ?>
		<?php } ?>
		</div>
...
if ( ! function_exists( 'blenco_entry_footer' ) ) {
	/**
	 * Blenco Entry Footer
	 *
	 * @return void
	 *
	 */
	function blenco_entry_footer() {
		if ( ! is_single() ) {
			if ( blenco_option( 'rt_blog_footer_visibility' ) ) { ?>
				<footer class="entry-footer rt-button">
					<a class="btn <?php echo esc_attr( blenco_option('rt_blog_btn_style') ); ?>"
					   href="<?php echo esc_url( get_permalink() ) ?>" <?php if( is_numeric(blenco_option('rt_blog_btn_radius') ) ) { ?>
						<?php } ?>>
						<span class="btn-icon">
			...
			if ( ( has_tag() && blenco_option( 'rt_single_tag_visibility' ) ) || blenco_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( blenco_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = blenco_option( 'rt_tags' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>

							<?php
							blenco_single_post_footer_meta(
								'content-below-meta', [ 'tag' ]
							);
							?>
						</div>
					<?php }
					if ( blenco_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = blenco_option( 'rt_social' ) ) {
								// printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>
							<?php PostShare::blenco_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'blenco_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function blenco_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'blenco_entry_profile' ) ) {
	/**
	 * Blenco Entry Profile
	 *
	 * @return void
	 *
	 */
	function blenco_entry_profile() {
		if ( blenco_option( 'rt_single_profile_visibility' ) ) {
					...
					<div class="profile-content">
						<div class="profile-author-info">
							...
							<div class="profile-info">
								...
								<?php if ( $prof_phone ) { ?>
									<span class="profile-phone"><?php echo esc_html__( 'Ph: ', 'blenco' ) ?><?php echo esc_html( $prof_phone );?></span>
								<?php } ?>
							</div>
						</div>
...
if ( ! function_exists( 'blenco_single_post_footer_meta' ) ) {
	/**
...
if ( ! function_exists( 'blenco_entry_content' ) ) {
	/**
	 * Entry Content
	 * @return void
	 */
...
if ( ! function_exists( 'blenco_sidebar' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function blenco_sidebar( $sidebar_id ) {
		...
		?>
		<aside id="sidebar" class="blenco-widget-area sidebar-sticky <?php echo esc_attr( $sidebar_cols ) ?>"
			   role="complementary">
...
if ( ! function_exists( 'blenco_post_class' ) ) {
	/**
	 * Get dynamic article classes
	 * @return string
	 */
	function blenco_post_class( $default_class = 'blenco-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . blenco_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= blenco_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = blenco_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= blenco_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = blenco_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . blenco_option( 'rt_blog_style' );
...
if ( ! function_exists( 'blenco_separate_meta' ) ) {
	/**
...
if ( ! function_exists( 'blenco_single_entry_header' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function blenco_single_entry_header() {
		?>
...
if ( ! function_exists( 'blenco_breadcrumb' ) ) {
	/**
	 * Blenco breadcrumb
	 * @return void
	 */
	function blenco_breadcrumb() {
		?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'Home', 'blenco' ) ?></a>
					<span class="raquo"><i class="icon-rt-user-datalist-feature"></i></span>
				</li>
				<li class="breadcrumb-item active" aria-current="page">
					<?php
					if ( is_tag() ) {
						esc_html_e( 'Posts Tagged: ', 'blenco' );
						?>
						...
						esc_html_e( 'Posts made in: ', 'blenco' );
						echo esc_html( get_the_time( is_year() ? 'Y' : ( is_month() ? 'F, Y' : 'F jS, Y' ) ) );
						echo '</span>';
					} elseif ( is_search() ) {
						echo '<span class="title">';
						esc_html_e( 'Search results for: ', 'blenco' );
						the_search_query();
						echo '</span>';
					} elseif ( is_404() ) {
						echo '<span class="title">';
						esc_html_e( '404', 'blenco' );
						echo '</span>';
					} elseif ( is_single() ) {
						...
						echo '<span class="title">';
						esc_html_e( 'Blog', 'blenco' );
						echo '</span>';
					} elseif ( class_exists( 'WooCommerce' ) and is_shop() ) {
						echo '<span class="title">';
						esc_html_e( 'Shop', 'blenco' );
						echo '</span>';
					}
					?>
				</li>
...
if ( ! function_exists( 'blenco_get_avatar_url' ) ) :
	function blenco_get_avatar_url( $get_avatar ) {
...
function blenco_comments_cbf( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback' : ?>
			<div class="pingback-entry"><span
					class="pingback-heading"><?php esc_html_e( 'Pingback:', 'blenco' ); ?></span> <?php comment_author_link(); ?>
			</div>
			<?php
			break;
		default :
			...
			<div class="comment-author">
				...
				<div class="author-info">
					...
					<div class="comment-details">
						...
						<?php
						$icon = blenco_get_svg( 'share' );
						// Display comment reply link
						comment_reply_link( array_merge( $args, [
							'add_below'  => $add_below,
							'depth'      => $depth,
							'max_depth'  => $args['max_depth'],
							'reply_text' => $icon . __( 'Reply', 'blenco' )
						] ) ); ?>
...
if ( ! function_exists( 'blenco_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function blenco_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( blenco_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( blenco_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
				<span class="btn-hamburger">
					<svg width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
						<circle cx="1.875" cy="1.875" r="1.875" fill="currentColor"/>
						<circle cx="9.5" cy="1.875" r="1.875" fill="currentColor"/>
						<circle cx="17.125" cy="1.875" r="1.875" fill="currentColor"/>
						<circle cx="1.875" cy="9.875" r="1.875" fill="currentColor"/>
						<circle cx="9.5" cy="9.875" r="1.875" fill="currentColor"/>
						<circle cx="17.125" cy="9.875" r="1.875" fill="currentColor"/>
						<circle cx="1.875" cy="17.875" r="1.875" fill="currentColor"/>
						<circle cx="9.5" cy="17.875" r="1.875" fill="currentColor"/>
						<circle cx="17.125" cy="17.875" r="1.875" fill="currentColor"/>
					</svg>
				</span>
			</button>
		</li>
...
if ( ! function_exists( 'blenco_hanburger2' ) ) {
	 ...
	 * @return void
	 */
	function blenco_hanburger2( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( blenco_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( blenco_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
				<span class="btn-hamburger btn-hamburger-bar">
					<svg width="20" height="14" viewBox="0 0 20 14" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M1 1H19" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
						<path d="M4 7H16" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
						<path d="M6 13H14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
					</svg>
				</span>
			</button>
		</li>
